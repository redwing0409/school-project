...
<%	
	...
	if(request.getAttribute("listArticle_ByCompositeQuery")!=null){
		list=(List<ArticleVO>)request.getAttribute("listArticle_ByCompositeQuery");
	}else{	
		list = articleSvc.getAllFront();
	}
...
<link rel="stylesheet" href="<%=request.getContextPath()%>/front_end/css/DiscussPage.css">
<link rel="stylesheet"
								...
								<td>
									<c:if test="${articleVO.member_no==account.member_no}">
										<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/article/article.do">
											<input type="button" class="btn btn-info deleteArticleBtn" value="刪除" id="deleteArticleBtn"> <input type="hidden" name="article_no" value="${articleVO.article_no}">
