...
<%
	SupVO SupVO = (SupVO) request.getAttribute("SupVO1"); //SupServlet.java (Concroller), 存入req的SupVO物件 (包括幫忙取出的empVO, 也包括輸入資料錯誤時的empVO物件)
%>
<html>
<head>
...
<link rel="stylesheet" href="<%=request.getContextPath()%>/supplier_end/sup/css/bootstrap.min.css">
...
<script>
   $(document).ready(function(){
	  $("#supdata").click(function(){
		 $("#sup_con").attr("value","Iversion");
		 $("#sup_telcode").attr("value","03");
		 $("#sup_tel").attr("value","4227151");
		 $("#sup_tax").attr("value","0911416931");
		 $("#sup_adcode").attr("value","320");
		 $("#sup_addr").attr("value","桃園市中壢區中央路232巷");
	  });
   });
</script>
</head>
		...
		<div class="row">
			...
			<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/sup/sup.do" 	name="form1">
				...
				<div style="margin-top:-35px">
					...
					<input type="button" id="supdata" value="magic">
				</div>
			</FORM>
		</div>
	</div>
